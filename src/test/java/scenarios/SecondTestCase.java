...
public class SecondTestCase extends SetUp {
	...
	/*
	 * ********************************************* IN THE MEHTOD
	 * newAccountDetailsAndTermsConditions() (I crated a json service to retrieve
	 * the data) SOMETIMES THE SERVICE GETS A 429 CODE AND IT CANT GET THE DATA FROM
	 * THE JSON, THE SERVICES SHOWS A 'TOO MANY REQUES IN THE BROWSER' RE RUN IT OR
	 * change the employee_name for the explicit way.
	 * **********************************************
	 */
	...
	public void setUp() throws IOException {
		...
		System.out.println(this.DBCoonection);
		...
		// SMall validation to pretend if we are getting the data from an specific database url or service url
		if(this.DBCoonection.contains("PROD_URL")) {
	...
	// **** Warning, sometimes this test could fail due a 429 while requesting the
	// json from url
	// 5. Fill Email field with the data from the API response
	// Firstname.Lastname@fake.com
	...
	public void newAccountDetailsAndTermsConditions() {
		...
		try {
			...
			Assert.assertTrue(createAccount.setAccountDetails(accountName, accountEmail),
					"Failed trying to create account.");
	...
	// 9. Following elements should be displayed: Getting Started, Wi-Fi and
	// Bluetooth, Device Software and Hardware, TroubleShooting
	@Test(priority = 5, enabled = true)
	public void validateEchoSupport() {
		HelpCustomerServicePage echoSupport = new HelpCustomerServicePage(driver);
		try {
			// To compare to list one given and other one retrieved, I use a equals() method
			// to validate and compare thw two lists.
			Assert.assertTrue(
					echoSupport.validateResults(supportItem1, supportItem2, supportItem3, supportItem4, supportItem5),
					"Failed trying to validate results.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
